...
def deploy_stack(config):
    ...
    try:
        ...
        deployment_name = config['deployment_name'] or 'daily-journal-deployment'
        should_update = True
        stack_waiter = 'stack_update_complete'
        try:
            cloudformation_client.describe_stacks(StackName=deployment_name)
        ...
        with open(os.path.join(os.getcwd(), os.path.dirname(__file__), 'cloud_formation.json')) as f:
            ...
            cloud_formation_settings['Resources']['StoreLambda']['Properties']['Code']['S3Bucket'] = deployment_bucket.name

            if should_update:
                print 'Updating stack'
                cloudformation_client.update_stack(StackName=deployment_name,
                                                   Capabilities=['CAPABILITY_NAMED_IAM'],
                                                   TemplateBody=json.dumps(cloud_formation_settings))
            else:
                print 'Creating stack'
                cloudformation_client.create_stack(StackName=deployment_name,
                                                   Capabilities=['CAPABILITY_NAMED_IAM'],
                                                   TemplateBody=json.dumps(cloud_formation_settings))
        waiter = cloudformation_client.get_waiter(stack_waiter)
        print 'Waiting for stack {0} to be {1}..'.format(deployment_name, 'update' if should_update else 'create')
        try:
            waiter.wait(StackName=deployment_name)
        except botocore.exceptions.ClientError as err:
            print 'Failed to deploy stack {}'.format(deployment_name)
            print err
    finally:
